...
public class NodoGET extends NodoOp0 {
	
	public NodoGET(String nom, int fila, int columna) {
		nombre = nom;
		this.fila = fila;
		this.columna = columna;
	}
	
	public NodoGET(String nom, int fila, int columna, NodoExpresion exp1, NodoExp1 exp2) {
		nombre = nom;
		this.fila = fila;
		this.columna = columna;
		expresion1 = exp1;
	...
	public String parseaTipos() throws Exception{
		...
		if (expresion1 != null && expresion2 != null) {
			if (expresion1.parseaTipos() != "bool" && expresion1.parseaTipos().equals(expresion2.parseaTipos())){
				return "bool";
			} else {
				throw new Exception("Tipos no compatibles en la operación " + nombre + " que está en la linea " + fila + " y columna " + columna +".");
			}
		} else {
			throw new Exception("Falta una expresión en la operación " + nombre + " que está en la linea " + fila + " y columna " + columna +".");
		}
	...
	public String generaCodigo() throws Exception {
		...
		s += "{" + (direccion + expresion1.getLongitud() + expresion2.getLongitud()) + "}geq;\n";
		return s;
	}
