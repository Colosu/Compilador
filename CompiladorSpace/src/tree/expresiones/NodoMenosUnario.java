...
public class NodoMenosUnario extends NodoOp4 {
	
	public NodoMenosUnario(String nom, int fila, int columna) {
		nombre = nom;
		this.fila = fila;
		this.columna = columna;
	}
	
	public NodoMenosUnario(String nom, int fila, int columna, NodoExp4 exp) {
		nombre = nom;
		this.fila = fila;
		this.columna = columna;
		expresion = exp;
	...
	public String parseaTipos() throws Exception{
		...
		if (expresion != null) {
			if (expresion.parseaTipos().equals("int") || expresion.parseaTipos().equals("float")){
	...
	public String generaCodigo() throws Exception {
		...
		s += "{" + (direccion + expresion.getLongitud()) + "}neg;\n";
		return s;
	}
